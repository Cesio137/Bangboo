...
export async function globalMessage( event: Events, member: GuildMember | PartialGuildMember, channel: TextChannel) {
...
export async function banMessage( event: Events, user: User, channel: TextChannel) {
    const canvas = new Canvas(1024, 260);
    const context = canvas.getContext("2d");
    const background = await loadImage(
        join(
    ...
    const { username } = user;
    const displayName = user.globalName || "Undefined";
