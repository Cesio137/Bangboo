...
use crate::menus::moderate::timeout::timeout_menu;
...
use serenity::all::{
    ...
    EditMember, GuildId, Member, MessageFlags, Timestamp, UserId,
};
...
pub async fn timeout_action(
    ...
    let timestamp =
        Timestamp::from_unix_timestamp((chrono::Utc::now() + duration).timestamp()).unwrap();

    for id in ids {
        let action = EditMember::new()
            .disable_communication_until(timestamp)
            .audit_log_reason(reason);

        match guild_id.edit_member(ctx.http(), id.clone(), action).await {
    ...
    if !failed.is_empty() {
        description.push("\n**Failed to timeout user(s):**".to_string());
...
pub async fn timeout_collector(
    ctx: &Context,
    ...
    let (embed, components) = timeout_menu(&member.user, &empty_slice, "");
    ...
    while let Some(i) = collector.next().await {
        ...
        match &i.data.kind {
            ComponentInteractionDataKind::Button => {
                ...
                if is_ok {
                    timeout_action(ctx, &i, guild_id, &ids, &duration, &reason).await;
                    timeout = false;
                    break;
                }
                edit = true;
        ...
        let (embed, components) = timeout_menu(&member.user, &ids, &duration);
        payload.embeds = Some(vec![embed]);
        payload.components = Some(components);
        if edit {
    ...
    if timeout || cancel {
        ...
        payload.embeds = Some(vec![close_embed.clone()]);
